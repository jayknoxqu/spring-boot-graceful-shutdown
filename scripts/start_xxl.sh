...
if [ "$JAR_PATH" == "" ]; then
  echo "用法：$0 <JAR_PATH> <SERVER_PORT> [PROFILE] [XXL_PORT] [PG_MEM] [HEAP_MEM]"
  exit 1
fi
...
XXL_CONFIG=""
if [ !-n"$XXL_PORT" ]; then
  XXL_CONFIG="--xxljob.executor.port=$XXL_PORT"
fi
...
echo -e "正在启动 ...\c"
nohup java $JAVA_OPTS $JAVA_MEM_OPTS -jar $JAR_PATH --spring.profiles.active=$PROFILE $XXL_CONFIG --server.port=$SERVER_PORT  > $JAR_PATH.$SERVER_PORT.log 2>&1 &
